...
chanData$weekday_is_saturday <- as.factor(chanData$weekday_is_saturday)
chanData$weekday_is_sunday <- as.factor(chanData$weekday_is_sunday)
chanData$is_weekend <- as.factor(chanData$is_weekend)
...
To Daniel: I we can add more plots especially those related to shares. Also boxplots of shares vs weekday_is_saturday, weekday_is_sunday and is_weekend for these three are categorical variables. Let me know what you think.
...
## Random Forest Model (I still have to fix this, I always get an error. =( I'm waiting for Dr.Post's answer.))
```{r}
ranForest <- train(shares ~., data = train, method = "rf",
