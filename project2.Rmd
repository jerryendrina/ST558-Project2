...
But since we are reporting by type of channel, then we can only use 16 for channel variables will become irrelevant. Below is the list of variables that we will use and their corresponding descriptions.  
...
# read in data set
data <- read_csv('OnlineNewsPopularity.csv') %>% select(-c(url,timedelta))
